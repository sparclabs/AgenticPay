"""Task13 Scenario 9: Luxury Watch - Sequential Two-Seller Per One Product Negotiation

Buyer negotiating with two dealers offering different luxury watches.
...
def main(model_name=None):
    ...
    buyer_max_price = 9500.0  # Maximum acceptable price for buyer (confidential) - investment collector budget
    seller1_min_price = 6800.0  # Minimum acceptable price for seller1 (confidential) - authorized dealer pricing
    seller2_min_price = 6500.0  # Minimum acceptable price for seller2 (confidential, slightly lower for gray market)
    ...
    env = Task3SequentialTwoSellerPerOneProductNegotiation(
        ...
        initial_seller2_price=8200.0,  # Initial price offered by seller2 (lower but gray market risk)
    ...
    user_profile = "Luxury watch collector and investor. Very knowledgeable about market values and authenticity. Prioritizes warranty and resale value. Risk-averse about counterfeits."
    print(f"User Profile: {user_profile}")
    
    # Get user requirement
    # Use default requirement for automatic running
    user_requirement = "Looking for Rolex Submariner Date in excellent condition. Must have box and papers."
    print(f"Using default requirement: {user_requirement}")
    
    # Reset environment with different products for each seller
    print("\n" + "="*60)
    print("Starting new sequential negotiation with two dealers (each with different sourcing and warranty)...")
    print("="*60)
    
    observation, info = env.reset(
        user_requirement=user_requirement,
        seller1_product_info={
            "name": "Rolex Submariner Date - Authorized Dealer",
            "model": "Submariner Date 126610LN",
            "year": "2023",
            "condition": "Excellent (9.5/10)",
            "box_and_papers": "Complete set",
            "warranty": "Rolex international warranty (2 years remaining)",
            "seller_type": "Authorized dealer",
            "authenticity_guarantee": "100% authentic, certified",
            "return_policy": "30 days full refund",
        },
        seller2_product_info={
            "name": "Rolex Submariner Date - Gray Market",
            "model": "Submariner Date 126610LN",
            "year": "2022",
            "condition": "Excellent (9/10)",
            "box_and_papers": "Complete set",
            "warranty": "Store warranty (1 year)",
            "seller_type": "Gray market dealer",
            "authenticity_guarantee": "Third-party authentication",
            "return_policy": "14 days, 15% restocking fee",
        },
    ...
    try:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("Task13 Scenario 9: Luxury Watch - Sequential Two-Seller Per One Product Negotiation Results\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Task13 Scenario 9: Luxury Watch - Sequential Two-Seller Per One Product Negotiation")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
