"""Task3 Close to Market Price Negotiation Example

Tests negotiation scenarios where seller_min_price is close to the market price
(product_info["price"]) to see if a deal can be reached. This is useful for
testing edge cases where the seller's bottom line is near the listed price.
"""
...
# Add project path
# Script is at: agenticpay/examples/single_buyer_product_seller/Task3_close_to_market_price_negotiation.py
# Need to go up 4 levels to reach project root
project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
sys.path.insert(0, project_root)

from agenticpay import make, Task3CloseToMarketPriceNegotiation  # Use registration system
...
def main(model_name=None):
    """Main function: Tests negotiation with seller_min_price close to market price
    ...
    # Task3: seller_min_price is close to market price to test if deal can be reached
    print("Creating agents...")
    seller_min_price = 175.0  # Minimum acceptable selling price for seller (confidential) - close to market price
    buyer_max_price = 200.0   # Maximum acceptable purchase price for buyer (confidential) - should be higher than market price
    ...
    print(f"Task3 Configuration:")
    print(f"  - Market Price (product list price): ${market_price:.2f}")
    print(f"  - Seller min price: ${seller_min_price:.2f}")
    print(f"  - Buyer max price: ${buyer_max_price:.2f}")
    print(f"  - Price difference from market: ${seller_min_price - market_price:.2f}")
    print(f"  - This tests if a deal can be reached when seller's bottom line is close to market price")
    ...
    env = make(
        ...
        initial_seller_price=185.0,  # Initial price offered by seller (slightly above market price)
    ...
    # env = Task3CloseToMarketPriceNegotiation(
    #     buyer_agent=buyer,
    #     seller_agent=seller,
    #     max_rounds=20,
    ...
    # Reset environment
    print("\n" + "="*60)
    print("Starting new negotiation (Task3: Close to Market Price Test)...")
    print("="*60)
    ...
    observation, info = env.reset(
        ...
        product_info={
            ...
            "brand": "Mountain Gear",
            "price": market_price,  # The product's market/list price
    ...
    while not done:
        ...
        if done:
            ...
            if agreed_price:
                print(f"\nTask3 Analysis:")
                print(f"  - Market Price (List Price): ${market_price:.2f}")
                ...
                if terminated:
                    ...
                    price_vs_market = agreed_price - market_price
                    print(f"  - Seller Profit: ${seller_profit:.2f}")
                    print(f"  - Buyer Savings: ${buyer_savings:.2f}")
                    print(f"  - Agreed Price vs Market: ${price_vs_market:+.2f} ({'above' if price_vs_market > 0 else 'below' if price_vs_market < 0 else 'equal to'} market price)")
                    print(f"  - Seller Min vs Market: ${seller_min_price - market_price:+.2f} ({'above' if seller_min_price > market_price else 'below' if seller_min_price < market_price else 'equal to'} market price)")
            print("="*60)
    ...
    try:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("Task3: Close to Market Price Negotiation Results\n")
            f.write("="*80 + "\n\n")
            ...
            f.write("\n")
            f.write("Task3 Analysis:\n")
            f.write(f"  Market Price (List Price): ${results['market_price']:.2f}\n")
            f.write(f"  Seller Min Price: ${results['seller_min_price']:.2f}\n")
            f.write(f"  Buyer Max Price: ${results['buyer_max_price']:.2f}\n")
            f.write(f"  Seller Min vs Market: ${results['seller_min_price'] - results['market_price']:+.2f}\n")
            if results.get('agreed_price'):
                ...
                if results.get('price_vs_market') is not None:
                    price_vs_market = results['price_vs_market']
                    f.write(f"  Agreed Price vs Market: ${price_vs_market:+.2f} ({'above' if price_vs_market > 0 else 'below' if price_vs_market < 0 else 'equal to'} market price)\n")
            f.write("\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Task3: Close to Market Price Negotiation")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
