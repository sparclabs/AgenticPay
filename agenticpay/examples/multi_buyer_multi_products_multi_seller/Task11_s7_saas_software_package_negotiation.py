"""Task11 Scenario 7: SaaS Software Package - Sequential Two-Buyer Two-Seller Two-Product Negotiation

Two buyers negotiating with two sellers for SaaS software packages (base license + support services).
Each buyer chooses one seller per round to negotiate with.
Prices represent total annual price for the software package (licenses + support).
...
def main(model_name=None):
    ...
    # buyer_max_price and seller_min_price represent total expected annual cost for the SaaS package (50 seats + support)
    # Based on scenario 3-1: buyer_max_price=$9,000/year (50 seats * $15/month), seller_min_price=$4,800/year (50 seats * $8/month)
    print("Creating agents...")
    buyer1_max_price = 9000.0  # Maximum acceptable total annual price for buyer1 (confidential, 50-person team budget)
    buyer2_max_price = 10500.0  # Maximum acceptable total annual price for buyer2 (confidential, larger team with premium support needs)
    seller1_min_price = 4800.0  # Minimum acceptable total annual price for seller1 (confidential, established SaaS provider)
    seller2_min_price = 4200.0  # Minimum acceptable total annual price for seller2 (confidential, newer competitor)
    ...
    env = Task3SequentialTwoBuyerTwoSellerTwoProductNegotiation(
        ...
        initial_seller1_price=8500.0,  # Initial total annual price offered by seller1 for the SaaS package
        initial_seller2_price=7500.0,  # Initial total annual price offered by seller2 for the SaaS package (competitive pricing)
        buyer1_max_price=buyer1_max_price,  # Buyer1 total max annual price (confidential, for SaaS package)
        buyer2_max_price=buyer2_max_price,  # Buyer2 total max annual price (confidential, for SaaS package)
        seller1_min_price=seller1_min_price,  # Seller1 total min annual price (confidential, for SaaS package)
        seller2_min_price=seller2_min_price,  # Seller2 total min annual price (confidential, for SaaS package)
        environment_info={
            "platform": "Direct sales / SaaS marketplace",
            "market_rate_range": "$8-$20 per seat/month",
    ...
    user_profile = "Two companies competing for project management software. Buyer1 is a 50-person development team seeking good value. Buyer2 is a 70-person team needing premium features and dedicated support. Both value scalability and integration capabilities."
    print(f"User Profile: {user_profile}")
    
    # Define two products with their individual prices
    # The product_info should contain a list of two products (base software license + support services package)
    # Based on scenario 3-1: ProjectFlow Pro - Annual License
    product_info = {
        "products": [
            {
                "name": "ProjectFlow Pro - Annual Software License",
                "brand": "SaaS Provider",
                "price": 7200.0,  # Individual annual price of base license (50 seats * $12/month * 12)
                "description": "Enterprise project management and collaboration platform",
                "base_price_per_seat_seller1": 20.0,  # $/month/seat list price
                "base_price_per_seat_seller2": 18.0,  # $/month/seat list price (competitive)
                "seats_needed": 50,
                "volume_discounts_seller1": {
                    "10-29 seats": "10% off",
                    "30-49 seats": "15% off",
                    "50-99 seats": "20% off",
                    "100+ seats": "30% off",
                },
                "volume_discounts_seller2": {
                    "10-29 seats": "12% off",
                    "30-49 seats": "18% off",
                    "50-99 seats": "25% off",
                    "100+ seats": "35% off",
                ...
                "contract_length_discount": "2-year: 5% off, 3-year: 10% off",
            },
            {
                "name": "Support & Services Package",
                "brand": "Customer Success",
                "price": 1800.0,  # Individual annual price of support services
                "support_tiers_seller1": {
                    "Basic": "Email support (included)",
                    "Priority": "$500/year - Phone + priority response",
                    "Dedicated": "$2000/year - Dedicated success manager + 24/7 support",
                },
                "support_tiers_seller2": {
                    "Basic": "Email support (included)",
                    "Priority": "$400/year - Email + chat support",
                    "Premium": "$1500/year - Dedicated support + training",
                },
                "included_seller1": [
                    "Onboarding and setup assistance",
                    "Monthly training webinars",
                    "Quarterly business reviews",
                    "Migration support from current tools",
                ],
                "included_seller2": [
                ...
                "migration_assistance": "Data import and team training",
    ...
    print(f"\nProducts (SaaS Package):")
    for i, p in enumerate(product_info["products"], 1):
        print(f"  {i}. {p['name']}: ${p['price']:.2f}/year")
    print(f"  Total Annual Package Price: ${total_product_price:.2f}")
    
    # Get user requirement (should describe purchasing the SaaS package)
    # Use default requirement for automatic running
    user_requirement = "Need a project management tool for our 50-person development team. Looking for annual subscription with good support, API integration, and scalability for future growth."
    print(f"Using default requirement: {user_requirement}")
    
    # Reset environment
    print("\n" + "="*60)
    print("Starting new sequential negotiation for SaaS software packages...")
    print("Two buyers competing with two sellers for Project Management SaaS")
    print("="*60)
    ...
    results = {
        "task": "Task11_s7_saas_software_package_negotiation",
    ...
    # Close environment
    env.close()
    print("\nSaaS software package negotiation completed!")
    ...
    try:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("Task11 Scenario 7: SaaS Software Package - Sequential Two-Buyer Two-Seller Two-Product Negotiation Results\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Task11 Scenario 7: SaaS Software Package - Sequential Two-Buyer Two-Seller Two-Product Negotiation")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
