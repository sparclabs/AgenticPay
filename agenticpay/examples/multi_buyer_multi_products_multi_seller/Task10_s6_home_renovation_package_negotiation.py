"""Task10 Scenario 6: Home Renovation Package - Sequential Two-Buyer Two-Seller Two-Product Negotiation

Two buyers negotiating with two sellers for home renovation packages (main work + additional services).
Each buyer chooses one seller per round to negotiate with.
Prices represent total price for the renovation package (core renovation + extras).
...
def main(model_name=None, difficulty="normal"):
    ...
    # buyer_max_price and seller_min_price represent total expected cost for the renovation package (main work + extras)
    # Based on scenario 2-3: buyer_max_price=$35,000, seller_min_price=$22,000
    print("Creating agents...")
    buyer1_max_price = 35000.0  # Maximum acceptable total renovation price for buyer1 (confidential, homeowner with budget limit)
    buyer2_max_price = 40000.0  # Maximum acceptable total renovation price for buyer2 (confidential, homeowner wanting premium quality)
    seller1_min_price = 22000.0  # Minimum acceptable total renovation price for seller1 (confidential, established contractor)
    seller2_min_price = 20000.0  # Minimum acceptable total renovation price for seller2 (confidential, smaller contractor)
    ...
    env = Task3SequentialTwoBuyerTwoSellerTwoProductNegotiation(
        ...
        initial_seller1_price=33000.0,  # Initial total price offered by seller1 for the renovation package (established contractor)
        initial_seller2_price=28000.0,  # Initial total price offered by seller2 for the renovation package (smaller contractor, lower)
        buyer1_max_price=buyer1_max_price,  # Buyer1 total max price (confidential, for renovation package)
        buyer2_max_price=buyer2_max_price,  # Buyer2 total max price (confidential, for renovation package)
        seller1_min_price=seller1_min_price,  # Seller1 total min price (confidential, for renovation package)
        seller2_min_price=seller2_min_price,  # Seller2 total min price (confidential, for renovation package)
        environment_info={
            "property_type": "Single family home",
            "property_age": "25 years",
            "peak_season": True,
            "permit_requirements": "Contractor handles permits",
        },
    ...
    user_profile = "Two homeowners competing for renovation services. Buyer1 is very concerned about hidden costs and needs quality work within budget. Buyer2 is willing to pay more for premium materials and faster completion. Both value reliability and warranty."
    print(f"User Profile: {user_profile}")
    
    # Define two products with their individual prices
    # The product_info should contain a list of two products (main renovation + additional services package)
    # Based on scenario 2-3: Full Home Renovation Package
    product_info = {
        "products": [
            {
                "name": "Core Renovation Work",
                "brand": "Licensed Contractor",
                "price": 28000.0,  # Individual price of main renovation work
                "square_footage": 1500,
                "scope_seller1": [
                    "Kitchen remodel (premium cabinets, quartz countertops, stainless appliances)",
                    "2 bathroom updates (new fixtures, tile work)",
                    "Flooring (1200 sq ft - engineered hardwood)",
                    "Interior painting (whole house - 2 coats)",
                ],
                "scope_seller2": [
                    "Kitchen remodel (standard cabinets, laminate countertops, standard appliances)",
                    "2 bathroom updates (basic fixtures, ceramic tile)",
                    "Flooring (1200 sq ft - laminate)",
                    "Interior painting (whole house - 1 coat)",
                ],
                "materials_grade_seller1": "Premium/Mid-high range",
                "materials_grade_seller2": "Standard/Mid range",
                "timeline_seller1": "8-10 weeks",
                "timeline_seller2": "10-12 weeks",
                "warranty_seller1": "2 year workmanship guarantee",
                "warranty_seller2": "1 year workmanship guarantee",
                "contractor_reviews_seller1": "4.8/5 (156 reviews, 15 years in business)",
                "contractor_reviews_seller2": "4.5/5 (67 reviews, 8 years in business)",
            },
            {
                "name": "Additional Services & Protections Package",
                "brand": "Supplementary Services",
                "price": 7000.0,  # Individual price of additional services
                "included_seller1": [
                    "Permit handling and inspections",
                    "Project management with weekly updates",
                    "Debris removal and site cleanup",
                    "Appliance installation and setup",
                    "Touch-up service (90 days post-completion)",
                    "Extended warranty option (5 years at +$2000)",
                ],
                "included_seller2": [
                    "Basic permit assistance",
                    "Bi-weekly check-ins",
                    "Final cleanup",
                    "Basic appliance hookup",
                ],
                "change_order_policy_seller1": "Written approval required, +10% markup",
                "change_order_policy_seller2": "Written approval required, +15% markup",
    ...
    # Get user requirement (should describe purchasing the renovation package)
    # Use default requirement for automatic running
    user_requirement = "Complete renovation of a 1500 sq ft home. Need quality work within reasonable budget. Very concerned about hidden costs and want clear timeline and warranty. Kitchen and bathrooms are priority."
    print(f"Using default requirement: {user_requirement}")
    
    # Reset environment
    print("\n" + "="*60)
    print("Starting new sequential negotiation for home renovation packages...")
    print("Two buyers competing with two sellers for Full Home Renovation")
    print("="*60)
    ...
    results = {
        ...
        "task": "Task10_s6_home_renovation_package_negotiation",
    ...
    # Close environment
    env.close()
    print("\nHome renovation package negotiation completed!")
    ...
    try:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("Task10 Scenario 6: Home Renovation Package - Sequential Two-Buyer Two-Seller Two-Product Negotiation Results\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Task10 Scenario 6: Home Renovation Package - Sequential Two-Buyer Two-Seller Two-Product Negotiation")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
