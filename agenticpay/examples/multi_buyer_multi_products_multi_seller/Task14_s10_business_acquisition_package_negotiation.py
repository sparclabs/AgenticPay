"""Task14 Scenario 10: Business Acquisition Package - Sequential Two-Buyer Two-Seller Two-Product Negotiation

Two buyers negotiating with two sellers for business acquisition packages (business assets + transition services).
Each buyer chooses one seller per round to negotiate with.
Prices represent total price for the acquisition package (business + support).
...
def main(model_name=None):
    ...
    # buyer_max_price and seller_min_price represent total expected cost for the acquisition package (business + transition)
    # Based on scenario 4-2: buyer_max_price=$120,000 (3x annual profit), seller_min_price=$80,000 (2x annual profit + psychological price)
    print("Creating agents...")
    buyer1_max_price = 120000.0  # Maximum acceptable total acquisition price for buyer1 (confidential, entrepreneur/investor)
    buyer2_max_price = 140000.0  # Maximum acceptable total acquisition price for buyer2 (confidential, experienced acquirer with strategic interest)
    seller1_min_price = 80000.0  # Minimum acceptable total selling price for seller1 (confidential, established 3-year business)
    seller2_min_price = 75000.0  # Minimum acceptable total selling price for seller2 (confidential, motivated seller)
    ...
    env = Task3SequentialTwoBuyerTwoSellerTwoProductNegotiation(
        ...
        initial_seller1_price=135000.0,  # Initial total price offered by seller1 for the acquisition package (3.5x annual profit asking)
        initial_seller2_price=120000.0,  # Initial total price offered by seller2 for the acquisition package (motivated seller, lower)
        buyer1_max_price=buyer1_max_price,  # Buyer1 total max price (confidential, for acquisition package)
        buyer2_max_price=buyer2_max_price,  # Buyer2 total max price (confidential, for acquisition package)
        seller1_min_price=seller1_min_price,  # Seller1 total min price (confidential, for acquisition package)
        seller2_min_price=seller2_min_price,  # Seller2 total min price (confidential, for acquisition package)
        environment_info={
            "platform": "Acquire.com",
            "seller_verified": True,
            "financials_verified": "By platform",
            "competing_buyers": 3,
            "time_on_market": "2-3 weeks",
        },
    ...
    user_profile = "Two entrepreneurs competing to acquire profitable e-commerce businesses. Buyer1 is a first-time acquirer looking for solid fundamentals. Buyer2 is an experienced operator willing to pay more for better metrics and synergies. Both need thorough due diligence and smooth transition."
    print(f"User Profile: {user_profile}")
    
    # Define two products with their individual prices
    # The product_info should contain a list of two products (business assets + transition services package)
    # Based on scenario 4-2: Profitable Shopify Store - Pet Supplies Niche
    product_info = {
        "products": [
            {
                "name": "E-commerce Business Assets",
                "brand": "Shopify Store",
                "price": 110000.0,  # Individual price of business assets
                "niche": "Pet Supplies",
                "business_age": "3 years",
                "monthly_revenue_seller1": 12000,
                "monthly_revenue_seller2": 10500,
                "monthly_profit_seller1": 3500,
                "monthly_profit_seller2": 3000,
                "annual_profit_seller1": 42000,
                "annual_profit_seller2": 36000,
                "profit_margin_seller1": "29%",
                "profit_margin_seller2": "29%",
                "traffic_source_seller1": {
                    "organic": "60%",
                    "paid_ads": "30%",
                    "social": "10%",
                },
                "traffic_source_seller2": {
                    "organic": "45%",
                    "paid_ads": "40%",
                    "social": "15%",
                },
                "assets_included": [
                    "Inventory ($5,000-7,000 value)",
                    "Supplier relationships (3-5 verified suppliers)",
                    "Email list (8,000-12,000 subscribers)",
                    "Social media accounts (Instagram, Facebook)",
                    "Brand trademarks and domain",
                    "Product photography library",
                ],
                "reason_for_selling": "Owner pursuing other business ventures",
                "asking_multiple": "3.2x annual profit",
            },
            {
                "name": "Transition & Support Package",
                "brand": "Seller Support Services",
                "price": 10000.0,  # Individual price of transition services
                "support_offered_seller1": "60 days comprehensive transition support with daily availability",
                "support_offered_seller2": "30 days transition support with weekly check-ins",
                "included_seller1": [
                    "Complete training on operations and suppliers",
                    "Customer service handover and training",
                    "Marketing strategy documentation",
                    "Supplier introductions and relationship transfer",
                    "Social media account management training",
                    "60-day post-sale email/call support",
                ],
                "included_seller2": [
                    "Basic operations training",
                    "Supplier contact list handover",
                    "30-day email support",
                ],
                "due_diligence_seller1": "Full access to: Google Analytics, Shopify backend, supplier agreements, tax returns (2 years)",
                "due_diligence_seller2": "Access to: Shopify reports, basic financials, supplier contacts",
                "non_compete_seller1": "2-year non-compete in pet supplies e-commerce",
                "non_compete_seller2": "1-year non-compete in pet supplies e-commerce",
                "escrow_terms": "90-day escrow period with performance milestones",
            },
        ]
    }
    ...
    # Get user requirement (should describe purchasing the acquisition package)
    # Use default requirement for automatic running
    user_requirement = "Looking to acquire a profitable e-commerce business with proven track record. Prefer established business (3+ years) with diversified traffic sources, solid profit margins, and strong fundamentals. Need thorough due diligence access and transition support."
    print(f"Using default requirement: {user_requirement}")
    
    # Reset environment
    print("\n" + "="*60)
    print("Starting new sequential negotiation for business acquisition packages...")
    print("Two buyers competing with two sellers for E-commerce Business")
    print("="*60)
    ...
    results = {
        "task": "Task14_s10_business_acquisition_package_negotiation",
    ...
    try:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("Task14 Scenario 10: Business Acquisition Package - Sequential Two-Buyer Two-Seller Two-Product Negotiation Results\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Task14 Scenario 10: Business Acquisition Package - Sequential Two-Buyer Two-Seller Two-Product Negotiation")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
