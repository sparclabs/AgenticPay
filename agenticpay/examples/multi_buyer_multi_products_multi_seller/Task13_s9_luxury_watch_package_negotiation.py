"""Task13 Scenario 9: Luxury Watch Package - Sequential Two-Buyer Two-Seller Two-Product Negotiation

Two buyers negotiating with two sellers for luxury watch packages (timepiece + authentication/warranty services).
Each buyer chooses one seller per round to negotiate with.
Prices represent total price for the watch package (watch + services).
...
def main(model_name=None):
    ...
    # buyer_max_price and seller_min_price represent total expected cost for the watch package (timepiece + services)
    # Based on scenario 4-1: buyer_max_price=$9,500, seller_min_price=$7,000
    print("Creating agents...")
    buyer1_max_price = 9500.0  # Maximum acceptable total watch price for buyer1 (confidential, watch enthusiast/collector)
    buyer2_max_price = 11000.0  # Maximum acceptable total watch price for buyer2 (confidential, serious collector willing to pay premium)
    seller1_min_price = 7000.0  # Minimum acceptable total selling price for seller1 (confidential, private collector with excellent condition watch)
    seller2_min_price = 6500.0  # Minimum acceptable total selling price for seller2 (confidential, dealer with good condition watch)
    ...
    env = Task3SequentialTwoBuyerTwoSellerTwoProductNegotiation(
        ...
        initial_seller1_price=9200.0,  # Initial total price offered by seller1 for the watch package (excellent condition, full set)
        initial_seller2_price=8500.0,  # Initial total price offered by seller2 for the watch package (good condition, partial set)
        buyer1_max_price=buyer1_max_price,  # Buyer1 total max price (confidential, for watch package)
        buyer2_max_price=buyer2_max_price,  # Buyer2 total max price (confidential, for watch package)
        seller1_min_price=seller1_min_price,  # Seller1 total min price (confidential, for watch package)
        seller2_min_price=seller2_min_price,  # Seller2 total min price (confidential, for watch package)
        environment_info={
            "platform": "Chrono24",
            "market_trend": "Prices stabilizing after 2022 peak",
            "escrow_available": True,
            "seller_verification": "Platform verified",
        },
    ...
    user_profile = "Two luxury watch buyers competing for Rolex Submariner. Buyer1 is a watch enthusiast focused on authenticity and condition. Buyer2 is a serious collector willing to pay premium for full box and papers. Both prioritize authenticity verification and provenance."
    print(f"User Profile: {user_profile}")
    
    # Define two products with their individual prices
    # The product_info should contain a list of two products (timepiece + authentication/services package)
    # Based on scenario 4-1: Rolex Submariner Date 116610LN
    product_info = {
        "products": [
            {
                "name": "Rolex Submariner Date 116610LN",
                "brand": "Rolex",
                "price": 8500.0,  # Individual price of the timepiece
                "year": 2019,
                "reference": "116610LN",
                "condition_seller1": "Excellent - Minor desk diving marks only",
                "condition_seller2": "Very Good - Light wear, well maintained",
                "box_papers_seller1": "Full set: Box, papers, warranty card (dated 2019)",
                "box_papers_seller2": "Watch only, no box or papers",
                "service_history_seller1": "Serviced by authorized Rolex service center in 2023 ($800 service)",
                "service_history_seller2": "No recent service, running well",
                "movement": "Caliber 3135 automatic",
                "case_size": "40mm stainless steel",
                "market_reference_price": 10500,
                "original_msrp": 9150,
                "seller_type_seller1": "Private collector (23 successful sales)",
                "seller_type_seller2": "Watch dealer (verified business)",
            },
            {
                "name": "Authentication & Protection Package",
                "brand": "Third-Party Services",
                "price": 1000.0,  # Individual price of services
                "authentication_seller1": [
                    "Platform verification completed",
                    "Third-party authentication certificate included",
                    "Serial number verification against Rolex database",
                    "Movement authenticity confirmed",
                ],
                "authentication_seller2": [
                    "Basic platform verification",
                    "Seller's authenticity guarantee",
                ],
                "warranty_seller1": "Seller provides 12-month mechanical warranty",
                "warranty_seller2": "Seller provides 6-month mechanical warranty",
                "insurance_seller1": "Fully insured shipping via FedEx (signature required)",
                "insurance_seller2": "Standard insured shipping",
                "return_policy_seller1": "7-day inspection period, full refund if not as described",
                "return_policy_seller2": "3-day inspection period, refund minus shipping",
                "escrow_service": "Available through platform ($200 fee)",
            },
        ]
    }
    ...
    # Get user requirement (should describe purchasing the watch package)
    # Use default requirement for automatic running
    user_requirement = "Looking for a Rolex Submariner Date 116610LN in excellent condition with full box and papers. Authenticity is my top priority. Want complete service history and proper verification."
    print(f"Using default requirement: {user_requirement}")
    
    # Reset environment
    print("\n" + "="*60)
    print("Starting new sequential negotiation for luxury watch packages...")
    print("Two buyers competing with two sellers for Rolex Submariner")
    print("="*60)
    ...
    results = {
        "task": "Task13_s9_luxury_watch_package_negotiation",
    ...
    # Close environment
    env.close()
    print("\nLuxury watch package negotiation completed!")
    ...
    try:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("Task13 Scenario 9: Luxury Watch Package - Sequential Two-Buyer Two-Seller Two-Product Negotiation Results\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Task13 Scenario 9: Luxury Watch Package - Sequential Two-Buyer Two-Seller Two-Product Negotiation")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
