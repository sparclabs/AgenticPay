"""Task14 Scenario 10: Business Acquisition Package - Sequential Two-Buyer Two-Product Negotiation

One seller negotiating with two buyers for business acquisition package (business assets + transition support).
Seller chooses one buyer per round to negotiate with.
Prices represent total price for the acquisition package.
...
def main(model_name=None, difficulty="normal"):
    ...
    # buyer_max_price and seller_min_price represent total expected cost for business acquisition package
    # Based on scenario 4-2: buyer_max_price=$120,000 (3× annual profit), seller_min_price=$80,000 (2× profit + psychology)
    print("Creating agents...")
    buyer1_max_price = 120000.0  # Maximum acceptable total price for buyer1 (confidential, first-time acquirer)
    buyer2_max_price = 140000.0  # Maximum acceptable total price for buyer2 (confidential, experienced operator)
    seller_min_price = 80000.0  # Minimum acceptable total price for seller (confidential, business owner)
    ...
    env = Task3SequentialTwoBuyerTwoProductNegotiation(
        ...
        initial_seller_price=135000.0,  # Initial total price offered by seller for acquisition package
    ...
    user_profile = "Two buyers competing for e-commerce business. Buyer1 is first-time acquirer with thorough due diligence focus. Buyer2 is experienced operator willing to pay premium for smooth transition."
    print(f"User Profile: {user_profile}")
    
    # Define two products with their individual prices
    # The product_info should contain a list of two products (business assets + transition support)
    product_info = {
        "products": [
            {
                "name": "E-commerce Business Assets",
                "brand": "Shopify Store",
                "price": 115000.0,  # Individual price of business assets
                "monthly_revenue": 12000,
                "monthly_profit": 3500,
                "profit_margin": "29%",
                "business_age": "3 years",
                "niche": "Pet supplies",
                "assets_included": ["Inventory ($5k)", "Supplier relationships", "Email list (8k)", "Social accounts", "Trademarks"],
            },
            {
                "name": "Transition & Support Package",
                "brand": "Seller",
                "price": 5000.0,  # Individual price of support services
                "services": ["30 days transition support", "Supplier introductions", "Operations training", "Marketing handover"],
                "reason_for_selling": "Owner pursuing other ventures",
                "traffic_sources": "60% organic, 30% paid, 10% social",
            },
        ]
    }
    ...
    user_requirement = "Looking to acquire profitable e-commerce business. Prefer established business with diversified traffic sources."
    print(f"Using default requirement: {user_requirement}")
    
    # Reset environment
    print("\n" + "="*60)
    print("Starting new sequential negotiation for business acquisition...")
    print("Seller choosing between two buyers for E-commerce Business Acquisition Package")
    print("="*60)
    ...
    try:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("Task14 Scenario 10: Business Acquisition Package - Sequential Two-Buyer Two-Product Negotiation Results\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Task14 Scenario 10: Business Acquisition Package - Sequential Two-Buyer Two-Product Negotiation")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
