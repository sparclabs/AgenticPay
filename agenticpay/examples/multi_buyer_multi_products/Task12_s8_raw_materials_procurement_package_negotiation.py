"""Task12 Scenario 8: Raw Materials Procurement Package - Sequential Two-Buyer Two-Product Negotiation

One seller negotiating with two buyers for raw materials package (bulk order + supply chain services).
Seller chooses one buyer per round to negotiate with.
Prices represent total price for the procurement package.
...
def main(model_name=None, difficulty="normal"):
    ...
    # buyer_max_price and seller_min_price represent total expected cost for raw materials package
    # Based on scenario 3-2: buyer_max_price=$4,500 (10k units × $0.45), seller_min_price=$3,200 (10k units × $0.32)
    print("Creating agents...")
    buyer1_max_price = 4500.0  # Maximum acceptable total price for buyer1 (confidential, electronics manufacturer)
    buyer2_max_price = 5000.0  # Maximum acceptable total price for buyer2 (confidential, larger volume/better terms)
    seller_min_price = 3200.0  # Minimum acceptable total price for seller (confidential, supplier cost)
    ...
    env = Task3SequentialTwoBuyerTwoProductNegotiation(
        ...
        initial_seller_price=4300.0,  # Initial total price offered by seller for procurement package
        ...
        environment_info={
            ...
            "supplier_evaluation": "New supplier being evaluated",
        },
    ...
    user_profile = "Two buyers competing for supplier. Buyer1 is electronics manufacturer with standard quality needs. Buyer2 is larger buyer willing to pay more for premium quality assurance."
    print(f"User Profile: {user_profile}")
    
    # Define two products with their individual prices
    # The product_info should contain a list of two products (bulk order + supply chain services)
    product_info = {
        "products": [
            {
                "name": "Ceramic Capacitors Bulk Order",
                "brand": "Supplier",
                "price": 3900.0,  # Individual price of bulk order (10,000 units × $0.39)
                "product_specs": "10μF, 0805 package",
                "quantity": 10000,
                "unit_price": 0.39,
                "lead_time": "2-4 weeks",
            ...
            },
            {
                ...
                "brand": "Supplier",
                "price": 600.0,  # Individual price of services
                "services": ["Quality inspection", "Flexible payment terms (Net 60)", "Priority production"],
                "defect_rate_guarantee": "< 0.1%",
                "supply_capacity": "500,000 units/month",
                "ongoing_support": "Dedicated account manager",
            },
        ]
    }
    ...
    user_requirement = "Need reliable supply of ceramic capacitors for manufacturing line. Initial order 10,000 units with ongoing monthly demand."
    print(f"Using default requirement: {user_requirement}")
    
    # Reset environment
    print("\n" + "="*60)
    print("Starting new sequential negotiation for raw materials...")
    print("Seller choosing between two buyers for Ceramic Capacitors Procurement Package")
    print("="*60)
    ...
    try:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("Task12 Scenario 8: Raw Materials Procurement Package - Sequential Two-Buyer Two-Product Negotiation Results\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Task12 Scenario 8: Raw Materials Procurement Package - Sequential Two-Buyer Two-Product Negotiation")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
