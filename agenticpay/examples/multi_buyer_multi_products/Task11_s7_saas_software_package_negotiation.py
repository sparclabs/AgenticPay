"""Task11 Scenario 7: SaaS Software Package - Sequential Two-Buyer Two-Product Negotiation

One seller negotiating with two buyers for SaaS software package (annual license + support/services).
Seller chooses one buyer per round to negotiate with.
Prices represent total annual price for the software package.
...
def main(model_name=None, difficulty="normal"):
    ...
    # buyer_max_price and seller_min_price represent total expected annual cost for SaaS package
    # Based on scenario 3-1: buyer_max_price=$9,000/year (50 seats × $15/month), seller_min_price=$4,800/year (50 seats × $8/month)
    print("Creating agents...")
    buyer1_max_price = 9000.0  # Maximum acceptable annual price for buyer1 (confidential, 50-person team)
    buyer2_max_price = 10500.0  # Maximum acceptable annual price for buyer2 (confidential, larger team/premium support)
    seller_min_price = 4800.0  # Minimum acceptable annual price for seller (confidential, base cost)
    buyer1_max_price, _ = adjust_zopa(buyer1_max_price, seller_min_price, difficulty)
    buyer2_max_price, _ = adjust_zopa(buyer2_max_price, seller_min_price, difficulty)
    ...
    env = Task3SequentialTwoBuyerTwoProductNegotiation(
        ...
        initial_seller_price=8500.0,  # Initial annual price offered by seller for SaaS package
    ...
    user_profile = "Two buyers competing for SaaS subscription. Buyer1 is 50-person team with standard needs. Buyer2 is larger team willing to pay more for priority support."
    print(f"User Profile: {user_profile}")
    
    # Define two products with their individual prices
    # The product_info should contain a list of two products (software license + support services)
    product_info = {
        "products": [
            {
                "name": "ProjectFlow Pro Annual Software License",
                "brand": "SaaS Vendor",
                "price": 7200.0,  # Individual annual price of license (50 seats with volume discount)
                "description": "Enterprise project management platform",
                "seats": 50,
                "base_price_per_seat_monthly": 20.0,
                "volume_discount": "20% off (50-99 seats tier)",
                "features": ["Unlimited projects", "Advanced analytics", "API access", "SSO"],
            },
            {
                "name": "Support & Services Package",
                "brand": "SaaS Vendor",
                "price": 1500.0,  # Individual annual price of support
                "support_tier": "Priority Support",
                "services": ["Priority ticket response", "Dedicated account manager", "Quarterly business reviews"],
                "contract_term": "1-year commitment",
    ...
    user_requirement = "Need project management tool for our 50-person team. Looking for annual subscription with good support."
    print(f"Using default requirement: {user_requirement}")
    
    # Reset environment
    print("\n" + "="*60)
    print("Starting new sequential negotiation for SaaS software...")
    print("Seller choosing between two buyers for ProjectFlow Pro Annual Package")
    print("="*60)
    ...
    try:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("Task11 Scenario 7: SaaS Software Package - Sequential Two-Buyer Two-Product Negotiation Results\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Task11 Scenario 7: SaaS Software Package - Sequential Two-Buyer Two-Product Negotiation")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
