"""Task13 Scenario 9: Luxury Watch Package - Sequential Two-Buyer Two-Product Negotiation

One seller negotiating with two buyers for luxury watch package (watch + authentication/protection).
Seller chooses one buyer per round to negotiate with.
Prices represent total price for the watch package.
...
def main(model_name=None, difficulty="normal"):
    ...
    # buyer_max_price and seller_min_price represent total expected cost for luxury watch package
    # Based on scenario 4-1: buyer_max_price=$9,500 (collector budget), seller_min_price=$7,000 (private seller bottom)
    print("Creating agents...")
    buyer1_max_price = 9500.0  # Maximum acceptable total price for buyer1 (confidential, watch enthusiast)
    buyer2_max_price = 11000.0  # Maximum acceptable total price for buyer2 (confidential, serious collector)
    seller_min_price = 7000.0  # Minimum acceptable total price for seller (confidential, private collector)
    ...
    env = Task3SequentialTwoBuyerTwoProductNegotiation(
        ...
        initial_seller_price=9200.0,  # Initial total price offered by seller for watch package
    ...
    user_profile = "Two buyers competing for luxury watch. Buyer1 is watch enthusiast with budget constraints. Buyer2 is serious collector willing to pay premium for full documentation and authentication."
    print(f"User Profile: {user_profile}")
    
    # Define two products with their individual prices
    # The product_info should contain a list of two products (watch + authentication/protection)
    product_info = {
        "products": [
            {
                "name": "Rolex Submariner Date 116610LN",
                "brand": "Rolex",
                "price": 8500.0,  # Individual price of the watch
                "year": 2019,
            ...
            },
            {
                ...
                "price": 700.0,  # Individual price of services
                "services": ["Professional authentication", "Escrow service", "Shipping insurance", "1-year buyer protection"],
                "authentication_provider": "Certified watchmaker verification",
    ...
    user_requirement = "Looking for Submariner Date in excellent condition with full box and papers. Authenticity is my top priority."
    print(f"Using default requirement: {user_requirement}")
    
    # Reset environment
    print("\n" + "="*60)
    print("Starting new sequential negotiation for luxury watch...")
    print("Seller choosing between two buyers for Rolex Submariner Package")
    print("="*60)
    ...
    try:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("Task13 Scenario 9: Luxury Watch Package - Sequential Two-Buyer Two-Product Negotiation Results\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Task13 Scenario 9: Luxury Watch Package - Sequential Two-Buyer Two-Product Negotiation")
    parser.add_argument(
        "--model",
        type=str,
        default=None,
