"""Task1: Price + Quality Negotiation Example

Agents negotiate on two parameters:
  - Price   (buyer wants lower, seller wants higher)
  - Quality (buyer wants Premium or better, seller prefers Standard)

Scenario: Industrial air compressor purchase — buyer is equipping a
manufacturing plant and needs reliable, high-quality equipment.
"""
...
SCENARIO_PARAMS = {
    # Tight ZOPA: only $2,500 spread (~11%) forces real price negotiation.
    # Seller floor $21k, buyer ceiling $23.5k — agents can't simply split the
    # difference and call it a day. Quality concessions must come with price moves.
    "price_range":      (21000.0, 23500.0),   # (seller_min, buyer_max)
    "quality_options":  ["Standard", "Premium", "Luxury"],
    # Standard = base model; Premium = enhanced filtration + remote monitoring;
    # Luxury = full smart-factory integration package
    "delivery_range":   (7, 60),
...
BUYER_PREFERENCES = ParamPreferences(
    price_limit=23500.0,        # hard ceiling — cannot exceed budget approval
    min_quality="Premium",       # needs remote monitoring capability
...
SELLER_PREFERENCES = ParamPreferences(
    price_limit=21000.0,         # cost floor for Premium unit — cannot go lower
    min_quality="Standard",      # Standard = lowest cost to seller
...
TASK_NAME     = "Task1_price_quality_negotiation"
ENV_ID        = "Task1_price_quality_negotiation-v0"
...
def main(model_name=None, difficulty="normal"):
    ...
    user_requirement = (
        ...
        "We have a target budget and quality expectations, and are open to finding the right balance."
    )
    user_profile = (
        "Procurement manager at a manufacturing firm. Evaluates trade-offs across price and quality "
