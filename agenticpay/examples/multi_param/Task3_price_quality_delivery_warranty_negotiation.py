"""Task3: Price + Quality + Delivery + Warranty Negotiation Example

Adds warranty period to Task2. The buyer wants at least 24 months of coverage
to stay within their maintenance budget cycle. The seller prefers the minimum
12-month standard warranty to reduce service liability. Agents can trade off
faster delivery or quality tier adjustments against warranty length.
"""
...
SCENARIO_PARAMS = {
    # Price ZOPA: $21k–$23.5k. Delivery gap: buyer needs ≤14 days, seller standard 28 days.
    # Warranty adds a 4th axis of tension: buyer needs 24 months, seller's standard is
    # 12 months — seller must stretch on both delivery and warranty, requiring price compensation.
    "price_range":      (21000.0, 23500.0),
    "quality_options":  ["Standard", "Premium", "Luxury"],
    "delivery_range":   (7, 30),
    "warranty_range":   (12, 36),   # 12 = seller standard, 36 = max they'll commit to
...
BUYER_PREFERENCES = ParamPreferences(
    ...
    min_warranty_months=24,         # 2-year cycle matches maintenance budget
...
SELLER_PREFERENCES = ParamPreferences(
    ...
    min_warranty_months=12,         # standard; longer warranty = more service cost
...
TASK_NAME     = "Task3_price_quality_delivery_warranty_negotiation"
ENV_ID        = "Task3_price_quality_delivery_warranty_negotiation-v0"
...
def main(model_name=None, difficulty="normal"):
    ...
    user_requirement = (
        ...
        "We have preferences across price, quality, delivery timeline, and warranty coverage, "
        "and are open to trade-offs between these dimensions."
    )
    user_profile = (
        "Procurement manager at a manufacturing firm. Evaluates total cost of ownership across "
        "price, quality, delivery, and warranty to arrive at the best overall deal."
    )
