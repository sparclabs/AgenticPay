"""Task15 Two-Buyer Two-Seller Multi-Product Bundle Quantity/Bulk-Discount Negotiation

Two buyers negotiate a product BUNDLE in parallel with two sellers.  Each seller
holds private tiered pricing for the bundle.  Both total quantity and effective
per-unit price are negotiable.  The best deal (highest combined buyer + seller
unit surplus) across all four conversation tracks is selected.

Price tags: BUYER_PRICE / SELLER_PRICE = TOTAL price for the entire bundle.
Quantity tags: ### BUYER_QUANTITY(X) ### / ### SELLER_QUANTITY(X) ###
  where X = total units across all products in the bundle.
"""
...
class Task15TwoBuyerTwoSellerMultiProductQuantityDiscountNegotiation(BaseEnv):
    """2B × 2S multi-product bundle quantity/bulk-discount negotiation.

    Four conversation tracks (b1s1, b1s2, b2s1, b2s2).  Agreement requires
    both effective-unit-price and total-quantity convergence per track.  The
    track with the highest combined unit surplus is selected as the final deal.
    """
    ...
    def reset(
        ...
        **kwargs: Any,
        ...
        # Accept either a products list or a single product_info dict
        bundle = products or (product_info.get("products") if product_info else None) or []
        display_info = {"products": bundle} if bundle else (product_info or {})

        qty = self.buyer_target_quantity

        buyer_suffix = (
            f"\nQUANTITY NEGOTIATION INSTRUCTIONS:\n"
            f"- You are negotiating a BUNDLE of products (total ~{qty} units). "
            f"You are comparing TWO suppliers.\n"
            f"- In EVERY turn include: ### BUYER_QUANTITY(X) ### and ### BUYER_PRICE($YYY) ###\n"
            f"- BUYER_PRICE is TOTAL price for the entire bundle.\n"
        )
        seller_suffix = (
            f"\nQUANTITY NEGOTIATION INSTRUCTIONS:\n"
            f"- You are selling a BUNDLE of products. Volume discounts available. "
            f"You negotiate with TWO buyers.\n"
            f"- In EVERY turn include: ### SELLER_QUANTITY(X) ### and ### SELLER_PRICE($YYY) ###\n"
            f"- SELLER_PRICE is TOTAL price for the entire bundle.\n"
        )
        ...
        b1max_total = self.buyer1_max_unit_price * qty if self.buyer1_max_unit_price else None
        b2max_total = self.buyer2_max_unit_price * qty if self.buyer2_max_unit_price else None
        s1min_total = self.seller1_min_unit_price * qty if self.seller1_min_unit_price else None
        s2min_total = self.seller2_min_unit_price * qty if self.seller2_min_unit_price else None

        self.buyer1_agent.initialize({
            "user_requirement": user_requirement,
            "max_price": b1max_total,
            "target_quantity": qty,
    ...
    def render(self, mode: str = "human") -> Optional[str]:
        ...
        lines.append(f"\n{'='*60}")
        lines.append(f"Round {self.current_round} — 2B×2S Bundle Negotiation")
        lines.append(f"{'='*60}")
        ...
        if self.selected_buyer and self.final_deal_price:
            aq = self.agreed_quantity or self.buyer_target_quantity
            lines.append(
                f"\n  DEAL: B{self.selected_buyer}xS{self.selected_seller}, "
                f"{aq} units @ ${self.final_deal_price/aq:.2f}/unit = ${self.final_deal_price:.2f}"
            )
        lines.append(f"{'='*60}\n")
        output = "\n".join(lines)
        if mode == "human":
    ...
    def _get_selected_min_unit_price(self) -> Optional[float]:
        if self.selected_seller == 1:
            return self.seller1_min_unit_price
        if self.selected_seller == 2:
            return self.seller2_min_unit_price
        return None
    ...
    def _calculate_reward(self) -> float:
        ...
        if self.negotiation_info.status == NegotiationStatus.AGREED and self.final_deal_price:
            ...
            print(
                f"Global Reward = bs({bs:.2f}) + sp({sp:.2f}) + tc({tc:.2f}) = {reward:.2f} "
                f"(B{self.selected_buyer}xS{self.selected_seller}, unit=${aunit:.2f}, qty={aq})"
            )
            return reward
