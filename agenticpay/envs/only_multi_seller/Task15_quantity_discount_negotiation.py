"""Task15 Parallel Two-Seller Quantity/Bulk-Discount Negotiation Environment

One buyer negotiates in parallel with two sellers for the same product.
Each seller holds private tiered pricing. Both quantity and per-unit price
are negotiable. The buyer picks the seller with the lower agreed unit price.
...
Scoring uses per-unit ZOPA (selected seller's ZOPA).
"""
...
class Task15ParallelTwoSellerQuantityDiscountNegotiation(BaseEnv):
    """One buyer vs two sellers; buyer picks seller with lower unit-price.

    Quantity and per-unit price are jointly negotiable. Each seller has
    independent tiered pricing and minimum unit-price floor.
    """
    ...
    def __init__(
        ...
        self.buyer_max_unit_price = buyer_max_unit_price
        ...
        # Quantity tracking
        self.buyer_qty_seller1: Optional[int] = None
        self.buyer_qty_seller2: Optional[int] = None
        ...
        self.final_deal_price: Optional[float] = None   # total
    ...
    def reset(
        ...
        **kwargs: Any,
        ...
        self.current_round = 0
        self.negotiation_info = NegotiationInfo()
        self.buyer_qty_seller1 = None
        self.buyer_qty_seller2 = None
        ...
        total_budget = self.buyer_max_unit_price * qty if self.buyer_max_unit_price else None
        ...
        buyer_suffix = (
            ...
            f"- Choose the supplier offering the best per-unit deal.\n"
        )
        seller_suffix = (
            f"\nQUANTITY NEGOTIATION INSTRUCTIONS:\n"
        ...
        self.seller1_agent.initialize({
            "product_info": product_info,
            "initial_price": self.initial_seller1_unit_price * qty,
            "min_price": self.seller1_min_unit_price * qty if self.seller1_min_unit_price else None,
        ...
        self.seller2_agent.initialize({
            "product_info": product_info,
            "initial_price": self.initial_seller2_unit_price * qty,
            "min_price": self.seller2_min_unit_price * qty if self.seller2_min_unit_price else None,
    ...
    def step(
        ...
        if buyer_action_seller1 is not None:
            ...
            if q: self.buyer_qty_seller1 = q
            elif self.buyer_qty_seller1 is None: self.buyer_qty_seller1 = self.buyer_target_quantity
        ...
        if buyer_action_seller2 is not None:
            ...
            if q: self.buyer_qty_seller2 = q
            elif self.buyer_qty_seller2 is None: self.buyer_qty_seller2 = self.buyer_target_quantity
        ...
        if seller1_action is not None:
            ...
            elif self.seller1_quantity is None: self.seller1_quantity = self.buyer_target_quantity
        ...
        if seller2_action is not None:
            ...
            elif self.seller2_quantity is None: self.seller2_quantity = self.buyer_target_quantity

        can_s1 = self._check_agreement_track(
            self.state_seller1, self.buyer_qty_seller1, self.seller1_quantity
        ) if buyer_action_seller1 is not None else False
        can_s2 = self._check_agreement_track(
            self.state_seller2, self.buyer_qty_seller2, self.seller2_quantity
        ) if buyer_action_seller2 is not None else False

        if can_s1 or can_s2:
            unit1 = self._effective_unit_price(self.state_seller1, self.buyer_qty_seller1, self.seller1_quantity) if can_s1 else None
            unit2 = self._effective_unit_price(self.state_seller2, self.buyer_qty_seller2, self.seller2_quantity) if can_s2 else None
        ...
        step_s1 = self._calculate_step_seller_reward(1)
        step_s2 = self._calculate_step_seller_reward(2)
        step_b = self._calculate_step_buyer_reward()

        observation = self._get_observation()
        info = self._get_info()
        info["step_seller1_reward"] = step_s1
        info["step_seller2_reward"] = step_s2
    ...
    def render(self, mode: str = "human") -> Optional[str]:
        ...
        lines.append(f"\n{'='*60}")
        lines.append(f"Round {self.current_round} - Parallel Two-Seller Negotiation")
        lines.append(f"{'='*60}")
        for sid, mem in [(1, self.memory_seller1), (2, self.memory_seller2)]:
        ...
        for sid, state, bq, sq in [
            (1, self.state_seller1, self.buyer_qty_seller1, self.seller1_quantity),
            (2, self.state_seller2, self.buyer_qty_seller2, self.seller2_quantity),
        ]:
            lines.append(f"\nSeller {sid}:")
            if state.buyer_price:
                bu = self._derive_unit_price(state.buyer_price, bq)
                lines.append(f"  Buyer: ${state.buyer_price:.2f} total  (${bu:.2f}/unit @ qty {bq or self.buyer_target_quantity})")
            if state.seller_price:
                su = self._derive_unit_price(state.seller_price, sq)
                lines.append(f"  Seller: ${state.seller_price:.2f} total  (${su:.2f}/unit @ qty {sq or self.buyer_target_quantity})")
    ...
    def _get_observation(self) -> Dict[str, Any]:
        return {
            ...
            "buyer_qty_seller1": self.buyer_qty_seller1,
            "buyer_qty_seller2": self.buyer_qty_seller2,
    ...
    # ------------------------------------------------------------------
    # Rewards (per-unit ZOPA)
    # ------------------------------------------------------------------

    def _get_selected_seller_min_unit_price(self) -> Optional[float]:
    ...
    def _calculate_reward(self) -> float:
        ...
        if self.negotiation_info.status == NegotiationStatus.AGREED and self.final_deal_price:
            ...
            reward = bs = sp = 0.0
            if self.buyer_max_unit_price: bs = (self.buyer_max_unit_price - aunit) * aq; reward += bs * self.reward_weights["buyer_savings"]
            smin = self._get_selected_seller_min_unit_price()
    ...
    def _calculate_step_buyer_reward(self) -> float:
        ...
        rewards = []
        for state, bq in [(self.state_seller1, self.buyer_qty_seller1), (self.state_seller2, self.buyer_qty_seller2)]:
            if state.buyer_price and self.buyer_max_unit_price:
                bu = self._derive_unit_price(state.buyer_price, bq)
                q = bq or self.buyer_target_quantity
                rewards.append((self.buyer_max_unit_price - bu) * q * self.reward_weights["buyer_savings"])
        agg = sum(rewards) / len(rewards) if rewards else 0.0
        return agg + rc * self.reward_weights["time_cost"]

    def _calculate_step_seller_reward(self, seller_id: int) -> float:
        rc = -self.current_round
        state = self.state_seller1 if seller_id == 1 else self.state_seller2
        sq = (self.seller1_quantity if seller_id == 1 else self.seller2_quantity) or self.buyer_target_quantity
        smin = self.seller1_min_unit_price if seller_id == 1 else self.seller2_min_unit_price
        reward = 0.0
        if state.seller_price and smin:
            su = self._derive_unit_price(state.seller_price, sq)
            reward += (su - smin) * sq * self.reward_weights["seller_profit"]
    ...
    def _calculate_global_score(self, print_details: bool = True) -> float:
        bmax = self.buyer_max_unit_price
        smin = self._get_selected_seller_min_unit_price()
