...
class Task3FiveProductNegotiation(BaseEnv):
    ...
    def reset(
        ...
        **kwargs: Any,
        ...
        # Seller gives initial offer (total price for all five products)
        product_names = [p.get("name", "Product") for p in products]
        # Format product names nicely
        if len(product_names) == 5:
            product_list = f"{product_names[0]}, {product_names[1]}, {product_names[2]}, {product_names[3]}, and {product_names[4]}"
        else:
            product_list = ", ".join(product_names[:-1]) + f", and {product_names[-1]}"
        initial_message = f"I'm offering {product_list} for a total of ${self.initial_seller_price:.2f}."
