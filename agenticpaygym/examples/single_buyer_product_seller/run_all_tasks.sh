...
if [ ${#MODELS[@]} -eq 0 ]; then
    ...
    for model in "${MODELS[@]}"; do
        ...
        python Task1_basic_price_negotiation.py --model "$model"
        
        echo ""
        echo "Running Task2 (model: $model)..."
        python Task2_close_price_negotiation.py --model "$model"
        
        echo ""
        echo "Running Task3 (model: $model)..."
        python Task3_close_to_market_price_negotiation.py --model "$model"
