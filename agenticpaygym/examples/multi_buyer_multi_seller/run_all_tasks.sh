...
RESULTS_BASE="$PROJECT_ROOT/agenticpaygym/results/multi_buyer_multi_seller"
...
if [ ${#MODELS[@]} -eq 0 ]; then
    ...
    echo "Running Task1..."
    python Task1_parallel_two_buyer_two_seller_negotiation.py 2>&1 | tee "$TEMP_LOG"
    
    echo ""
    echo "Running Task2..."
    python Task2_parallel_three_buyer_three_seller_negotiation.py 2>&1 | tee "$TEMP_LOG"
    
    echo ""
    echo "Running Task3..."
    python Task3_sequential_two_buyer_two_seller_negotiation.py 2>&1 | tee "$TEMP_LOG"
    
    echo ""
    echo "Running Task4..."
    python Task4_sequential_three_buyer_three_seller_negotiation.py 2>&1 | tee "$TEMP_LOG"
    ...
    for model in "${MODELS[@]}"; do
        ...
        python Task1_parallel_two_buyer_two_seller_negotiation.py --model "$model" 2>&1 | tee "$TEMP_LOG"
        save_run_history "$TEMP_LOG" "$model" "Task1"
        
        echo ""
        echo "Running Task2 (model: $model)..."
        python Task2_parallel_three_buyer_three_seller_negotiation.py --model "$model" 2>&1 | tee "$TEMP_LOG"
        save_run_history "$TEMP_LOG" "$model" "Task2"
        
        echo ""
        echo "Running Task3 (model: $model)..."
        python Task3_sequential_two_buyer_two_seller_negotiation.py --model "$model" 2>&1 | tee "$TEMP_LOG"
        save_run_history "$TEMP_LOG" "$model" "Task3"
        
        echo ""
        echo "Running Task4 (model: $model)..."
        python Task4_sequential_three_buyer_three_seller_negotiation.py --model "$model" 2>&1 | tee "$TEMP_LOG"
