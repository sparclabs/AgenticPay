...
if [ ${#MODELS[@]} -eq 0 ]; then
    ...
    for model in "${MODELS[@]}"; do
        ...
        python Task1_parallel_two_buyer_two_seller_negotiation.py --model "$model"
        
        echo ""
        echo "Running Task2 (model: $model)..."
        python Task2_parallel_three_buyer_three_seller_negotiation.py --model "$model"
        
        echo ""
        echo "Running Task3 (model: $model)..."
        python Task3_sequential_two_buyer_two_seller_negotiation.py --model "$model"
        
        echo ""
        echo "Running Task4 (model: $model)..."
        python Task4_sequential_three_buyer_three_seller_negotiation.py --model "$model"
