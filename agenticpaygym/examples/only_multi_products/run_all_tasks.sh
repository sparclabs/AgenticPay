...
if [ ${#MODELS[@]} -eq 0 ]; then
    ...
    for model in "${MODELS[@]}"; do
        ...
        python Task1_multi_product_negotiation.py --model "$model"
        
        echo ""
        echo "Running Task2 (model: $model)..."
        python Task2_two_product_negotiation.py --model "$model"
        
        echo ""
        echo "Running Task3 (model: $model)..."
        python Task3_five_product_negotiation.py --model "$model"
        
        echo ""
        echo "Running Task4 (model: $model)..."
        python Task4_select_three_from_five_negotiation.py --model "$model"
